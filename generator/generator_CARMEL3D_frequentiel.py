...
#----------------------------------------------------------------------------------------
   ...
   def generMATERIAL_DIELECTRIC(self,obj):
       ...
       if 'PERMEABILITY' not in obj.valeur or 'PERMITTIVITY' not in obj.valeur:
            print "ERREUR! Le matériau non conducteur (dielectric) de nom %s doit contenir les propriétés PERMEABILITY et PERMITTIVITY." % obj.get_sdname()
            raise ValueError, tr("ERREUR! Le materiau non conducteur (DIELECTRIC) de nom %s doit contenir les proprietes PERMEABILITY et PERMITTIVITY." % obj.get_sdname())
       else:
