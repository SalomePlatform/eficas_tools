...
INITIALIZATION=PROC(nom="INITIALIZATION",op=None,
     ...
     Title = SIMP( statut='o',typ='TXM',fr='Titre du cas etudie. Ce titre figurera sur les dessins.',ang='Title of the case being considered. This title shall be marked on the drawings.'),
...
INITIAL_STATE=PROC(nom="INITIAL_STATE",op=None,
        Initial_Conditions=SIMP(statut='o',typ='TXM',into=['ZERO ELEVATION', 'CONSTANT ELEVATION','ZERO DEPTH','CONSTANT DEPTH','SPECIAL','TPXO SATELLITE ALTIMETRY'],defaut='ZERO ELEVATION',
         ...
         b_initial_elevation = BLOC (condition = "Initial_Conditions == 'CONSTANT ELEVATION'",
           Initial_Elevation       = SIMP(statut='o',typ='R',fr='Valeur utilisee avec l''option :      CONDITIONS INITIALES - COTE CONSTANTE',ang='Value to be used with the option : INITIAL CONDITIONS  -CONSTANT ELEVATION' ),
         ),
         b_initial_depth     = BLOC (condition = "Initial_Conditions == 'CONSTANT DEPTH'",
           Initial_Depth       = SIMP(statut='o',typ='R',fr='Valeur utilisee avec l''option : CONDITIONS INITIALES :-HAUTEUR CONSTANTE-',ang='Value to be used along with the option:          INITIAL CONDITIONS -CONSTANT DEPTH-' ),
         ),
...
# Il faut seulement l un des 3
        Liquid_Boundaries=FACT(statut='f',max='**',
            ...
            Options=SIMP(statut='f',typ='I',into=['classical boundary conditions','Thompson method based on characteristics'],fr='On donne 1 entier par frontiere liquide      1 : conditions aux limites classiques      2 : methode de Thompson avec calcul de caracteristiques',ang='One integer per liquid boundary is given      1 : classical boundary conditions      2 : Thompson method based on characteristics'),
            ...
            Type_Condition=SIMP(statut='o',typ='TXM',into=['Flowrates','Elevations','Velocity'],),
             b_Flowrates     = BLOC (condition = "Type_Condition == 'Flowrates'",
                Prescribed_Flowrates=SIMP(statut='o',typ='R',fr=' Valeurs des debits imposes aux frontieres liquides entrantes. Lire la partie du mode d''emploi consacree aux conditions aux limites',ang='Values of prescribed flowrates at the inflow boundaries. The section about boundary conditions is to be read in the manual'),
             ),
             b_Elevations   = BLOC (condition = "Type_Condition == 'Elevations'",
                Prescribed_Elevations=SIMP(statut='o',typ='R',fr='Valeurs des cotes imposees aux frontieres liquides entrantes. Lire la partie du mode d''emploi consacree aux conditions aux limites',ang='Values of prescribed elevations at the inflow boundaries. The section about boundary conditions is to be read in the manual'),
             ),
             b_Velocity   = BLOC (condition = "Type_Condition == 'Velocity'",
                Prescribed_Velocity=SIMP(statut='o',typ='R',fr='Valeurs des vitesses imposees aux frontieres liquides entrantes. Lire la partie du mode d''emploi consacree aux conditions aux limites',ang='Values of prescribed velocities at the liquid inflow boundaries. Refer to the section dealing with the boundary conditions'),
         ),
       ),
